...
// 摄像头默认参数配置 在此修改摄像头配置
//--------------------------------------------------------------------------------------------------
#define MT9V03X_W               (240)                                           // 图像宽度     范围 [1-752]
#define MT9V03X_H               (135)                                           // 图像高度     范围 [1-480]
...
#define MT9V03X_EXP_TIME_DEF    (800)                                           // 曝光时间         摄像头收到后会自动计算出最大曝光时间，如果设置过大则设置为计算出来的最大曝光值
#define MT9V03X_FPS_DEF         (10 )                                           // 图像帧率         摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
//attention 图像的帧数决定单位是时间处理图片的多少,过多可能会导致图像处理不完整
